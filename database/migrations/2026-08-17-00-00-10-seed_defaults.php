...
INSERT INTO users (id, username, email, password_hash, role) VALUES
    ...
    (12, 'x', 'x@bestofxyz.local', '$2y$12$e0MYzX12458019348912389124801923849012389012389012389', 'user');
...
INSERT INTO resources (id, category_id, submitted_by, title, slug, url, url_hash, host, description, score, hot_score, created_at) VALUES
    (100, 1, 2, 'The Rust Book', 'rust-book', 'https://doc.rust-lang.org/book/', UNHEX(SHA2('https://doc.rust-lang.org/book/', 256)), 'doc.rust-lang.org', 'The definitive guide to Rust, written and maintained by the Rust team itself. Starts from zero and builds up through ownership, lifetimes, and the trickier parts of the type system, with runnable examples throughout.', 402, 402.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 4 MONTH)),
    (101, 1, 3, 'Structure and Interpretation of Computer Programs (SICP)', 'sicp', 'https://mitpress.mit.edu/sicp/', UNHEX(SHA2('https://mitpress.mit.edu/sicp/', 256)), 'mitpress.mit.edu', 'MIT’s foundational classic on computer programming, functional computational models, and abstraction techniques.', 385, 385.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 3 MONTH)),
    (102, 1, 4, 'Crafting Interpreters', 'crafting-interpreters', 'https://craftinginterpreters.com/', UNHEX(SHA2('https://craftinginterpreters.com/', 256)), 'craftinginterpreters.com', 'A handbook for making programming languages by Bob Nystrom, walking through tree-walk interpreters and bytecode virtual machines.', 310, 310.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 2 MONTH)),
    (103, 1, 5, 'Visual Studio Code', 'vscode', 'https://code.visualstudio.com/', UNHEX(SHA2('https://code.visualstudio.com/', 256)), 'code.visualstudio.com', 'Extensible, lightweight open-source code editor with a massive ecosystem for debugging, syntax, and Git integration.', 278, 278.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 1 MONTH)),
    (104, 1, 6, 'Full Stack Open', 'fullstack-open', 'https://fullstackopen.com/', UNHEX(SHA2('https://fullstackopen.com/', 256)), 'fullstackopen.com', 'University of Helsinki’s comprehensive modern web development course covering React, Node.js, TypeScript, and GraphQL.', 245, 245.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 2 WEEK)),
    (105, 1, 7, 'Exercism', 'exercism', 'https://exercism.org/', UNHEX(SHA2('https://exercism.org/', 256)), 'exercism.org', 'Code practice and human mentoring for 70+ programming languages. Completely free and community-powered.', 198, 198.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 1 DAY));

-- Resources for Best Rust Courses (category_id = 10)
INSERT INTO resources (id, category_id, submitted_by, title, slug, url, url_hash, host, description, score, hot_score, created_at) VALUES
    (1, 10, 2, 'The Rust Book', 'rust-book', 'https://doc.rust-lang.org/book/', UNHEX(SHA2('https://doc.rust-lang.org/book/', 256)), 'doc.rust-lang.org', 'The definitive guide to Rust, written and maintained by the Rust team itself. Starts from zero and builds up through ownership, lifetimes, and the trickier parts of the type system, with runnable examples throughout.', 214, 214.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 4 MONTH)),
    (2, 10, 1, 'Rust by Example', 'rust-by-example', 'https://doc.rust-lang.org/rust-by-example/', UNHEX(SHA2('https://doc.rust-lang.org/rust-by-example/', 256)), 'doc.rust-lang.org', 'A collection of runnable examples that exercise various Rust concepts and standard libraries.', 186, 186.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 3 MONTH)),
    (3, 10, 1, 'Rustlings', 'rustlings', 'https://github.com/rust-lang/rustlings', UNHEX(SHA2('https://github.com/rust-lang/rustlings', 256)), 'github.com', 'Small exercises to get you used to reading and writing Rust code. Great hands-on practice companion to The Book.', 152, 152.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 2 MONTH)),
    (4, 10, 1, 'Comprehensive Rust', 'comprehensive-rust', 'https://google.github.io/comprehensive-rust/', UNHEX(SHA2('https://google.github.io/comprehensive-rust/', 256)), 'google.github.io', 'A multi-day Rust course developed by the Android team at Google.', 98, 98.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 1 MONTH)),
    (5, 10, 1, 'Zero To Production In Rust', 'zero-to-production-in-rust', 'https://www.zero2prod.com/', UNHEX(SHA2('https://zero2prod.com/', 256)), 'zero2prod.com', 'An opinionated guide to backend development in Rust by Luca Palmieri.', 74, 74.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 2 WEEK));

-- Resources for Pending Category: Best Rust Courses for Beginners (category_id = 13)
INSERT INTO resources (id, category_id, submitted_by, title, slug, url, url_hash, host, description, score, hot_score, created_at) VALUES
    (6, 13, 8, 'Rust for Absolute Beginners', 'rust-for-absolute-beginners', 'https://freecodecamp.org/news/rust-for-absolute-beginners', UNHEX(SHA2('https://freecodecamp.org/news/rust-for-absolute-beginners', 256)), 'freecodecamp.org', 'Assumes no prior programming knowledge at all — starts with what a variable even is.', 18, 18.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 3 DAY)),
    (7, 13, 9, 'Learn Rust the Slow Way', 'learn-rust-the-slow-way', 'https://blog.example.com/rust-slow-way', UNHEX(SHA2('https://blog.example.com/rust-slow-way', 256)), 'indie blog series', 'A weekly blog series aimed squarely at people learning to program for the first time.', 9, 9.0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 1 DAY));
...
INSERT INTO resource_tags (resource_id, tag_id) VALUES
    ...
    (100, 1), (100, 7), (100, 13), -- free, official, comprehensive
    ...
    (1, 1), (1, 7), (1, 13),       -- free, official, comprehensive
    (2, 1), (2, 5), (2, 7),        -- free, interactive, official
    (3, 1), (3, 8), (3, 9),        -- free, exercises, beginner
    (4, 1), (4, 7), (4, 9),        -- free, official, beginner
    (5, 2), (5, 4), (5, 10),       -- paid, book, advanced
    -- Pending category resources
    (6, 1), (6, 3), (6, 9),        -- free, video, beginner
    (7, 1), (7, 9);                -- free, beginner

-- Comments for Resource #100 and #1 (The Rust Book)
INSERT INTO comments (id, resource_id, parent_id, user_id, body, score, depth, created_at) VALUES
    (1, 100, NULL, 10, 'Genuinely the best intro out there — the ownership chapter is where it finally clicked for me. Read three other tutorials before this one and none of them made it stick.', 12, 0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 3 WEEK)),
    (2, 100, 1,    2,  'Same, chapter 4 did it for me too. Ended up re-reading it twice before it actually stuck.', 4, 1, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 3 WEEK)),
    (3, 100, NULL, 11, 'Good, but chapter 8 (collections) assumes you already know a fair bit. Wouldn\'t call it a true zero-to-hero path on its own.', 3, 0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 1 MONTH)),
    (4, 100, NULL, 12, 'meh', -2, 0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 2 MONTH)),
    (5, 100, NULL, 9,  'Just started this today, wish me luck!', 0, 0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 2 MINUTE)),
    (6, 1,   NULL, 10, 'Genuinely the best intro out there — the ownership chapter is where it finally clicked for me. Read three other tutorials before this one and none of them made it stick.', 12, 0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 3 WEEK)),
    (7, 1,   6,    2,  'Same, chapter 4 did it for me too. Ended up re-reading it twice before it actually stuck.', 4, 1, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 3 WEEK)),
    (8, 1,   NULL, 11, 'Good, but chapter 8 (collections) assumes you already know a fair bit. Wouldn\'t call it a true zero-to-hero path on its own.', 3, 0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 1 MONTH)),
    (9, 1,   NULL, 12, 'meh', -2, 0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 2 MONTH)),
    (10, 1,  NULL, 9,  'Just started this today, wish me luck!', 0, 0, DATE_SUB(UTC_TIMESTAMP(), INTERVAL 2 MINUTE));
SQL;
